...
{
    /// <summary>
    /// A tap gesture detects one or more consecutive taps
    /// </summary>
    public class TapGestureRecognizer : GestureRecognizer
    {
        ...
        private async void FailGestureAfterDelayIfNoTap()
        {
            await Task.Delay(TimeSpan.FromSeconds(ThresholdSeconds));

			VerifyFailGestureAfterDelay();
        }
		...
		{
			MainThreadCallback(ThresholdSeconds, VerifyFailGestureAfterDelay);
		}
